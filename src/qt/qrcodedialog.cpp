...
{
    QString ret = QString("modjo:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non MODJO unit input in lnReqAmount, we generate the URI with MODJO as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(modjoUnits::format(modjoUnits::MODJO, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
